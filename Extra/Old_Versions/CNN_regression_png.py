...
torch.save(model.state_dict(), f"{os.path.basename(os.getcwd())}_{image_type}_{len(dataset)}samples_{num_epochs}ep_{train_batch_size}bs_noroot.pth")
...
with torch.no_grad():
    for img1, img2, img3, target_coords in test_loader:
        ...
        test_loss_Chamfer = lossChamfer.item()

print("Error medio de test Chamfer: ", test_loss_Chamfer)