import math
import numpy as np
from data.openRCS.rcs_functions import *
            ...
            for m in range(ntria): # test to see if front face is illuminated
                ...
                if iflag==0:
                    if (ilum[m]==1 and ndotk>=1e-5) or ilum[m]==0:
                        ...
                        _, sump, _, _ = calculaCampos(Area, cfac2, corel, th2, wave,Jy2,Ic,uu,vv,ww,phr,sumt,sump,sumdt,sumdp, m, Jx2, T1, T2)
            lista_sump.append(sump)
    return lista_sump
